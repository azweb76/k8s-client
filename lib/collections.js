...
var factory = module.exports = function (request) {
    ...
    var Collection = function (collection, schema, innerCollections, options) {
        ...
        this.getBySelector = function(selector, callback){
          var labelSelector = utils.toSelector(selector);
          var endpoint = getPath(collection) + '/?' + qs.stringify({ labelSelector: labelSelector });
          request({
              endpoint: endpoint
              , method: 'GET'
              , options : options
        ...
        this.getBy = function (query, callback) {
            ...
            **/
            ...
            if (query.name) {
               endpoint = collection + '/' + query.name;
            }
            request({
                endpoint: endpoint
                , method: 'GET'
        ...
        this.deleteBySelector = function deleteBySelector(selector, callback){
          var self = this;
          self.getBySelector(selector, function(err, results){
            if(err) return callback(err);
            async.forEach(results.items, function(item, cb){
              self.delete(item.metadata.name, cb);
            }, function(err){
              if(err) return callback(err);
